...
namespace FileManager.Controllers
{
	...
	public class ManageController : Controller
	{
		...
		{
			var context = new ApplicationDbContext();
			var user = context.Users.First(item => item.Id == id);
			var roles = new List<string>();
			foreach (var role in context.Roles)
			{
				roles.Add(role.Name);
			}
			var model = new IndexViewModel { Id = user.Id, Email = user.Email, Roles = roles };
			return View(model);
		}
		...
		public ActionResult AddRole(string id, string role)
		{
			...
			{
				UserManager.RemoveFromRole(id, context.Roles.First(item=>item.Id == roleItem.RoleId).Name);
			}
			UserManager.AddToRole(id, role);
			return Redirect("/Manage/ManageRoles");
		}
		//
		// GET: /Manage/Index
